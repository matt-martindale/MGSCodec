...
class CodecCallArray {
    var codecCalls: [CodecCall] = []
    lazy var codecCallsContainer = codecCalls
    
    init(_ codecCalls: [CodecCall]) {
        self.codecCalls = codecCalls
        reloadCodecCalls()
    }
    
    func reloadCodecCalls() {
        self.codecCallsContainer = codecCalls
        self.codecCallsContainer.shuffle()
    }
    
    func getRandomCodecCall() -> CodecCall {
        if self.codecCallsContainer.isEmpty {
            reloadCodecCalls()
        }
        return codecCallsContainer.removeLast()
    }
}

var snakeCodecCallsArray = [
